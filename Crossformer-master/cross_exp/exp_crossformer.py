...
class Exp_crossformer(Exp_Basic):
    ...
    def train(self, setting):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            if early_stopping.early_stop:
                ...
                # plt.figure()
                # plt.plot(np.array(range(len(train_loss_all))), train_loss_all, color='blue',
                #          label='train_loss')
    ...
    def eval(self, setting, save_pred = False, inverse = False):
        ...
        if (save_pred):
            ...
            print(preds.size)
            print(preds[0][1][6])
            plt.plot(np.array(range(96,96+len(preds[0]))), [i[6] for i in preds[97]], color='blue', label='preds')
            plt.plot(np.array(range(len(trues[0])*2)), [i[6] for i in trues[1]]+[i[6] for i in trues[97]], color='yellow', label='trues')
            plt.title(f'--{len(preds[1])} times --')
            plt.legend(loc='upper right')
            plt.show()
